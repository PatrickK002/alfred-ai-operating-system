...
export default function App() {
  ...
  const [view, setView] = useState("today"); // today | closet | add | looks
    ...
    <div style={{ minHeight: "100vh", background: S.paper, color: S.ink, fontFamily: "Georgia, 'Times New Roman', serif" }}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .btn-primary:hover { background:${S.aubergineDeep}; }
        .btn-ghost { background:transparent; color:${S.aubergine}; border:1px solid ${S.aubergine}55; }
        .btn-ghost:hover { background:${S.blushSoft}; }
        select, input { font-family:system-ui,sans-serif; font-size:13px; padding:6px 8px; border:1px solid ${S.aubergine}33; border-radius:2px; background:#fff; color:${S.ink}; }
      ...
      <header style={{ background: S.aubergine, padding: "22px 20px 0" }}>
        <div style={{ maxWidth: 960, margin: "0 auto" }}>
          ...
          <nav style={{ display: "flex", gap: 22, marginTop: 16 }}>
      ...
      <main style={{ maxWidth: 960, margin: "0 auto", padding: "26px 20px 60px" }}>
        {view === "today" && (
          <Today weather={weather} weatherErr={weatherErr} loadingW={loadingW} getWeather={getWeather}
            ...
            saveCurrentLook={saveCurrentLook} currentLookSaved={currentLookSaved} />
        )}
        {view === "looks" && (
...
// ---------- Today view ----------
function Today({ weather, weatherErr, loadingW, getWeather, occasion, setOccasion, buildOutfit, outfit, items, setView, saveCurrentLook, currentLookSaved }) {
  return (
    <div>
      {/* Weather strip */}
            ...
            <div style={{ padding: "10px 12px" }}>
              {editId === i.id ? (
                ...
                <div>
                  ...
                  <button className="btn btn-ghost" style={{ padding:"6px 12px" }} onClick={()=>setEditId(i.id)}>Edit tags</button>
                </div>
              )}
            </div>
          </div>
      ...
      {options.map(o => (
        <button key={o} type="button" onClick={() => onSelect(o)}
          style={{
            ...
            padding: "7px 12px", borderRadius: 3, transition: ".12s",
